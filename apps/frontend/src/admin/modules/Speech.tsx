...
const LIGHTTTS_VOICES = [
  { voice: "debate_voice_1", label: "辩论正式音色 1 · prompt voice" },
  { voice: "debate_voice_2", label: "辩论正式音色 2 · prompt voice" },
  { voice: "debate_voice_3", label: "辩论正式音色 3 · prompt voice" },
  { voice: "debate_voice_4", label: "辩论正式音色 4 · prompt voice" },
];
...
function normalizeProvider(value: unknown, kind: "asr" | "tts"): SpeechProvider {
  ...
  if (provider === "lighttts" && kind === "tts") return "lighttts";
  return "alicloud";
}
...
function providerDefaults(kind: "asr" | "tts", provider: SpeechProvider) {
  ...
  if (provider === "lighttts") return kind === "tts" ? LIGHTTTS_TTS_DEFAULTS : ALICLOUD_ASR_DEFAULTS;
...
function SectionEditor({
  ...
  const localProvider = draft.provider === "funasr" || draft.provider === "local_qwen" || draft.provider === "lighttts";
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="space-y-1.5">
            ...
            >
              ...
              {kind === "tts" && <option value="lighttts">本机 LightTTS / CosyVoice</option>}
            </Select>
          </div>
          <div className="space-y-1.5">
            <Label>模型</Label>
...
function VoicePresetManager({
  ...
  const baseVoices =
    config.tts.provider === "xfyun"
      ...
      : config.tts.provider === "local_qwen"
        ? LOCAL_QWEN_VOICES
        : config.tts.provider === "lighttts"
          ? LIGHTTTS_VOICES
          : ALICLOUD_VOICES;
  const allowCustomVoice = config.tts.provider !== "local_qwen" && config.tts.provider !== "lighttts";
  ...
  function openEditor(preset: VoicePreset | null) {
    ...
    if ((config.tts.provider === "local_qwen" || config.tts.provider === "lighttts") && next.voice && !isBaseVoice(next.voice)) {
      next = { ...next, voice: config.tts.provider === "lighttts" ? "debate_voice_1" : "dylan", enabled: false };
    }
    setEditing(next);
  ...
  async function savePreset() {
    ...
    if ((config.tts.provider === "local_qwen" || config.tts.provider === "lighttts") && !isBaseVoice(editing.voice)) {
      toast(config.tts.provider === "lighttts" ? "LightTTS 目前只保留 4 个固定 prompt 音色" : "本机 Qwen 目前只保留 Aiden / Ryan / Dylan / Sohee 四个音色", "error");
      return;
    }
    setSaving(true);
    try {
        ...
        {editing && (
          <div className="space-y-3 rounded-md border border-border bg-muted/30 p-4">
            <div className="grid gap-3 md:grid-cols-3">
                    ...
                    placeholder={
                      config.tts.provider === "xfyun"
                        ...
                        : config.tts.provider === "local_qwen"
                          ...
                          : config.tts.provider === "lighttts"
                            ? "LightTTS 固定音色：debate_voice_1 / 2 / 3 / 4"
              ...
              <div className="space-y-1.5">
                <Label>服务商</Label>
                <Input value={config.tts.provider === "xfyun" ? "讯飞" : config.tts.provider === "local_qwen" ? "本机 Qwen" : config.tts.provider === "lighttts" ? "本机 LightTTS / CosyVoice" : "阿里云百炼 Qwen"} disabled />
              </div>
            </div>
...
function pcmRateFromMime(mime: string): number {
  const match = /(?:rate|sample_rate)=(\d+)/i.exec(mime || "");
  const value = match ? Number(match[1]) : NaN;
  return Number.isFinite(value) && value >= 8000 && value <= 192000 ? value : 0;
}

// 把原始 16bit 小端 单声道 PCM 包成最小 WAV 容器，让 <audio> 能直接播放
// （LightTTS/CosyVoice 返回 audio/L16 原始 PCM，浏览器无法直接播放原始流）。
function pcmL16ToWavBytes(pcm: Uint8Array, sampleRate: number): Uint8Array {
  const dataLen = pcm.byteLength - (pcm.byteLength % 2);
  ...
  new Uint8Array(buffer, 44).set(pcm.subarray(0, dataLen));
    ...
    ws.onmessage = (event) => {
      ...
      if (message.type === "ready") {
        ...
        if (!bytes) {
          ...
          const reportedMime = String(message.mime_type || mime || "audio/mpeg");
          const isPcm = /l16|pcm|audio\/raw/i.test(reportedMime);
          let playBytes: Uint8Array = merged;
          let playType = reportedMime;
          if (isPcm) {
            // 原始 PCM 包成 WAV，浏览器才能播放（与大屏归档播放使用的格式一致）。
            const sampleRate = pcmRateFromMime(reportedMime) || Number(selected?.sample_rate) || 24000;
            playBytes = pcmL16ToWavBytes(merged, sampleRate);
            playType = "audio/wav";
          }
          const audio = new Audio();
          const blob = new Blob([playBytes.buffer as ArrayBuffer], { type: playType });

...
def _agent_tts_chain_log_level_name() -> str:
    for key in ("PHDEBATE_AGENT_TTS_CHAIN_LOG_LEVEL", "PHDEBATE_LOG_LEVEL", "LOG_LEVEL"):
...
def _agent_tts_chain_debug_enabled() -> bool:
    return _agent_tts_chain_log_level_name() == "DEBUG"
...
def configure_agent_tts_chain_logging() -> None:
    ...
    if not _agent_tts_chain_debug_enabled():
        logger.setLevel(logging.INFO)
        setattr(logger, "_phdebate_file_logging_configured", True)
        setattr(logger, "_phdebate_file_logging_path", "")
        return
    ...
    try:
        ...
        if not getattr(logger, "_phdebate_startup_marker_logged", False):
            ...
            logger.debug("agent_tts_chain startup_marker %s", json.dumps(marker_payload, ensure_ascii=False, default=str))
            logger.debug("agent_tts_chain startup_marker log_file=%s", log_file)
            logger.debug("agent_tts_chain startup_marker ready=1")
...
class MatchStore:
        ...
        self.snapshot: Dict[str, Any] = {
            ...
            "clocks": [
                ...
                },
                {
                    ...
                    "remaining_ms": 26000,
                    "state": "running",
                    "deadline_at": to_iso(now + timedelta(milliseconds=26000)),
                },
                {
                    "id": "clock_neg_total",
                    "phase_id": "phase_free_debate",
    ...
    def _demo_phases(self) -> List[Dict[str, Any]]:
        rows = [
            ...
            ("phase_free_debate", "free_debate", "自由辩论", "free_debate", "neutral", None, 360),
        ...
        for index, (phase_id, key, name, phase_type, side, seat, duration) in enumerate(rows, start=1):
            ...
            if phase_type == "free_debate":
                phase["side_total_seconds"] = 180  # 每一方 3 分钟
                phase["turn_seconds"] = 30  # 单次发言 30 秒
            phases.append(phase)
        return phases
    ...
    def _phases_from_ruleset(self, ruleset: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for index, node in enumerate(ruleset.get("flow", []) or [], start=1):
            ...
            duration = int(node.get("duration_seconds") or (360 if phase_type == "free_debate" else 180))
            ...
            if phase_type == "free_debate":
                side_total = int(node.get("side_total_seconds") or (180 if duration in {240, 360, 480} else max(1, duration // 2)))
                phase["side_total_seconds"] = side_total
                phase["turn_seconds"] = int(node.get("turn_seconds") or 30)  # 默认单次发言 30 秒
            ...
            for index, generated_phase in enumerate(generated):
                ...
                if generated_phase.get("phase_type") == "free_debate":
                    phase["side_total_seconds"] = int(
                        ...
                        or max(1, int(generated_phase.get("duration_seconds") or 360) // 2)
                    )
                    phase["turn_seconds"] = int(generated_phase.get("turn_seconds") or 30)
    ...
    def _fallback_text_for_timing(
        ...
        *,
        ...
        budget = self._agent_output_budget(time_limit, self._speaker_speech_rate(speaker), speaker=speaker)
    ...
    def fallback_status(self) -> Dict[str, Any]:
        ...
        checks = {
            ...
            "free_debate_rules_ready": bool(free_phase and self._free_side_total_seconds(free_phase) == 180 and self._free_turn_seconds(free_phase) == 30),
    ...
    async def run_agent_command(self, speaker_id: str, command: str, prompt: str = "") -> Dict[str, Any]:
        ...
        cmd_budget = self._agent_output_budget(60, self._speaker_speech_rate(speaker), speaker=speaker)
        payload = {
    ...
    def _ensure_runtime_fields(self) -> None:
        ...
        for phase in self.snapshot.get("phases", []):
            ...
            if phase.get("phase_type") != "free_debate":
                continue
            duration = int(phase.get("duration_seconds") or 360)
            if "side_total_seconds" not in phase:
                side_total = 180 if duration in {240, 360, 480} else max(1, duration // 2)
                phase["side_total_seconds"] = side_total
            else:
                side_total = int(phase.get("side_total_seconds") or 180)
                if side_total in {120, 240} and duration in {240, 360, 480}:
                    side_total = 180
                    phase["side_total_seconds"] = side_total
            phase["duration_seconds"] = side_total * 2
            if phase.get("turn_seconds") in {None, "", 15}:
                phase["turn_seconds"] = 30
        if current_phase:
            self._sync_current_phase_clocks_after_config(current_phase)
        for segment in self.snapshot.get("recent_transcript", []):
    ...
    def _build_agent_payload(
        ...
        budget = self._agent_output_budget(time_limit, speech_rate, speaker=speaker)
        ...
        return {
            ...
            "min_len": budget["min_chars"],
            "min_length": budget["min_chars"],
            "target_min_chars": budget["min_chars"],
    ...
    def _build_self_intro_payload(self, task_id: str, speech_id: str, speaker: Dict[str, Any]) -> Dict[str, Any]:
        ...
        speech_rate = self._speaker_speech_rate(speaker)
        budget = self._agent_output_budget(time_limit, speech_rate, speaker=speaker)
        budget.update({"char_budget": 20, "target_chars": 20, "max_token": 20, "min_chars": 12})
        budget["length_hint"] = self._agent_length_hint(budget["min_chars"], budget["target_chars"])
        holder = "正方" if speaker["side"] == "affirmative" else "反方"
        model_id = str(config.get("model_id") or config.get("model_name") or "").strip()
        model_display_name = str(config.get("model_name") or speaker.get("model_name") or "").strip()
        return {
    ...
    async def _synthesize_sentence_tts(
        ...
        try:
            archive_dir = self._audio_archive_dir(match_id, phase_key, speech_id)
            archive_dir.mkdir(parents=True, exist_ok=True)
            archive_audio = result.audio
            archive_mime_type = result.mime_type
            if self._is_pcm_tts_mime(result.mime_type):
                archive_audio = self._pcm16le_to_wav_bytes(result.audio, sample_rate=self._pcm_sample_rate_from_mime(result.mime_type))
                archive_mime_type = "audio/wav"
            ext = self._audio_extension(archive_mime_type)
            ...
            if sentence_idx == 0 and archive_mime_type == "audio/mpeg":
        ...
        if provider == "lighttts":
            ...
            for key in (
                ...
                if options.get(key) is None or options.get(key) == "":
                    ...
                    if value is not None and value != "":
                        options[key] = value
        if provider in {"local_qwen", "alicloud", "lighttts"}:
    ...
    def _tts_live_mime_type(self, selection: Any) -> str:
        ...
        if raw in {"0", "false", "no", "off"}:
            return ""
        if provider not in {"alicloud", "lighttts"}:
            return ""
        ...
        if mime_type == "audio/mpeg":
            return mime_type
        if provider == "lighttts" and self._is_pcm_tts_mime(mime_type):
            return mime_type
        return ""
    ...
    def _speaker_tts_preset(self, speaker: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        if not speaker:
            return None
        provider = str((integration_config.active_section("tts") or {}).get("provider") or "alicloud")
        preset_id = str(speaker.get("tts_voice_preset_id") or "").strip()
    ...
    def _speaker_tts_setting_float(
        self,
        ...
        preset = self._speaker_tts_preset(speaker)
        if preset and preset.get(key) not in {None, ""}:
            try:
                return max(low, min(high, float(preset[key])))
            except (TypeError, ValueError):
                pass
        return self._tts_setting_float(key, env_name, default, low, high)

    def _speaker_tts_speaking_cps(self, speaker: Optional[Dict[str, Any]]) -> float:
        return self._speaker_tts_setting_float(speaker, "tts_speaking_cps", "PHDEBATE_TTS_SPEAKING_CPS", 5.4, 2.0, 8.0)
    ...
    def _speaker_agent_max_token_margin(self, speaker: Optional[Dict[str, Any]]) -> float:
        return self._speaker_tts_setting_float(speaker, "agent_max_token_margin", "PHDEBATE_AGENT_MAX_TOKEN_MARGIN", 1.0, 1.0, 2.0)
    ...
    def _speaker_speech_rate(self, speaker: Dict[str, Any]) -> float:
        ...
        preset = self._speaker_tts_preset(speaker)
        if str(speaker.get("speaker_type") or "") == "agent" and not preset:
    ...
    def _speaker_agent_speech_time_factor(self, speaker: Optional[Dict[str, Any]]) -> float:
        return self._speaker_tts_setting_float(speaker, "agent_speech_time_factor", "PHDEBATE_AGENT_SPEECH_TIME_FACTOR", 0.78, 0.50, 1.0)

    def _agent_output_budget(self, time_limit_seconds: int, speech_rate: float, *, speaker: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        """
        raw_cps = self._speaker_tts_speaking_cps(speaker)
        playback_rate = self._screen_tts_playback_rate()
        cps = raw_cps * playback_rate
        factor = self._speaker_agent_speech_time_factor(speaker)
        ...
        min_chars = max(1, min(target_chars, int(round(target_chars * 0.9))))
        raw_tokens = char_budget * self._agent_tokens_per_char() * self._speaker_agent_max_token_margin(speaker)
        ...
        return {
            ...
            "min_chars": min_chars,
            "length_hint": self._agent_length_hint(min_chars, target_chars),
            "max_token": max_token,
        }

    def _agent_length_hint(self, min_chars: int, target_chars: int) -> str:
        return f"请控制在 {min_chars} 到 {target_chars} 个中文字之间，尽量接近 {target_chars} 字，不要明显短于 {min_chars} 字。"
    ...
    def _clamp_agent_text_to_budget(self, text: Any, payload: Dict[str, Any]) -> tuple[str, bool]:
        ...
        if limit <= 0 or len(raw) <= limit:
            return raw, False
        hard_limit = max(limit + 12, int(round(limit * 1.10)))
        window = raw[: min(len(raw), hard_limit)].rstrip()
        if not window:
            return "", True

        target_tail = window[limit:]
        for offset, ch in enumerate(target_tail):
            if ch in "。！？!?；;\n":
                return window[: limit + offset + 1].strip(), True
    ...
    @staticmethod
    def _is_pcm_tts_mime(mime_type: str) -> bool:
        value = (mime_type or "").lower()
        return "l16" in value or "pcm" in value or "audio/raw" in value or value == "application/octet-stream"

    @staticmethod
    def _pcm_sample_rate_from_mime(mime_type: str) -> int:
        match = re.search(r"(?:rate|sample_rate)=(\d+)", mime_type or "", re.IGNORECASE)
        if not match:
            return 24000
        try:
            ...
            return 24000
        return sample_rate if 8000 <= sample_rate <= 192000 else 24000

    def _pcm16le_to_wav_bytes(self, audio: bytes, *, sample_rate: int) -> bytes:
        if len(audio) % 2:
            audio = audio[:-1]
        audio = self._trim_pcm16le_silence(audio, sample_rate=sample_rate)
        buffer = io.BytesIO()
    ...
    def _trim_pcm16le_silence(self, audio: bytes, *, sample_rate: int) -> bytes:
        """Trim model-added silence from independent LightTTS sentence chunks.

        Each LightTTS sentence is archived as a standalone WAV. The model often
        returns 200-1000ms of lead silence per request; if we keep it, the screen
        hears that silence at every sentence boundary. Keep a small cushion so
        consonant attacks and natural sentence endings are not clipped.
        """
        if not audio:
            return audio
        enabled = str(os.getenv("PHDEBATE_TTS_TRIM_PCM_SILENCE", "1")).strip().lower()
        if enabled in {"0", "false", "no", "off"}:
            return audio
        try:
            threshold = int(os.getenv("PHDEBATE_TTS_TRIM_SILENCE_THRESHOLD", "120").strip())
        except ValueError:
            threshold = 120
        threshold = max(1, min(2000, threshold))
        try:
            keep_ms = int(os.getenv("PHDEBATE_TTS_TRIM_SILENCE_KEEP_MS", "60").strip())
        except ValueError:
            keep_ms = 60
        keep_ms = max(0, min(300, keep_ms))
        sample_rate = max(8000, min(192000, int(sample_rate or 24000)))
        if len(audio) % 2:
            audio = audio[:-1]
        frame_count = len(audio) // 2
        if frame_count <= 0:
            return audio
        chunk_frames = max(1, sample_rate // 100)  # 10ms analysis window
        keep_frames = int(sample_rate * keep_ms / 1000)
        threshold_sq = threshold * threshold

        def has_signal(start_frame: int, end_frame: int) -> bool:
            start = max(0, start_frame) * 2
            end = min(frame_count, end_frame) * 2
            count = max(0, (end - start) // 2)
            if count <= 0:
                return False
            total = 0
            for offset in range(start, end, 2):
                ...
                total += sample * sample
            return total >= threshold_sq * count

        first = 0
        found = False
        for start in range(0, frame_count, chunk_frames):
            end = min(frame_count, start + chunk_frames)
            if has_signal(start, end):
                first = max(0, start - keep_frames)
                found = True
                break
        if not found:
            return audio
        ...
        last = frame_count
        for end in range(frame_count, 0, -chunk_frames):
            start = max(0, end - chunk_frames)
            if has_signal(start, end):
                last = min(frame_count, end + keep_frames)
                break
        if last <= first:
            return audio
        return audio[first * 2 : last * 2]
    ...
    def _speech_section_ready(self, kind: str) -> bool:
        ...
        if provider in {"funasr", "local_qwen", "lighttts"}:

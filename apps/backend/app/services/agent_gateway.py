...
class AgentGateway:
    ...
    def _build_openai_messages(self, payload: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        target_chars = self._positive_int(payload.get("target_chars"), 400)
        min_chars = self._target_min_chars(payload, target_chars)
        ...
        system_prompt = (
            ...
            f"请用中文进行辩论发言，发言时间 {time_limit} 秒，建议字数 {min_chars} 到 {target_chars} 字，"
            f"尽量接近 {target_chars} 字，不要明显短于 {min_chars} 字，"
    ...
    def _positive_int(self, raw: Any, fallback: int) -> int:
        try:
            value = int(float(raw))
        except (TypeError, ValueError):
            return fallback
    ...
    def _target_min_chars(self, payload: Dict[str, Any], target_chars: int) -> int:
        other_info = payload.get("other_info") if isinstance(payload.get("other_info"), dict) else {}
        for raw in (
            payload.get("target_min_chars"),
            payload.get("min_len"),
            payload.get("min_length"),
            other_info.get("target_min_chars"),
            other_info.get("min_chars"),
        ):
            value = self._positive_int(raw, 0)
            if 0 < value <= target_chars:
                return value
        return max(1, min(target_chars, int(round(target_chars * 0.9))))

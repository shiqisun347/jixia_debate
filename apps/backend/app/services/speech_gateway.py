...
def select_tts_gateway(
    *,
    ...
    if provider == "lighttts":
        return SpeechGatewaySelection(
            gateway=LightTTSTTSGateway(section=section, preset=preset),
            provider="lighttts",
            options={},
            preset=preset,
        )
...
class LightTTSTTSGateway:
    ...
    def stream_mime_type(self, **options: Any) -> str:
        session = self._session_options(options)
        sample_rate = int(session.get("sample_rate") or 24000)
        return f"audio/L16;rate={sample_rate}"

    async def synthesize_stream(self, text: str, **options: Any) -> AsyncIterator[Dict[str, Any]]:
        content = normalize_tts_text(text)
        if not content:
            raise SpeechGatewayError("TTS 合成文本不能为空。", code="empty_text", provider="lighttts")
        session = self._session_options(options)
        endpoint = _join_http_url(str(self.section.get("endpoint") or "http://127.0.0.1:8080"), "/inference_zero_shot")
        prompt_wav_path = str(session.get("prompt_wav_path") or "").strip()
        if not prompt_wav_path:
            raise SpeechGatewayError("LightTTS 缺少 prompt_wav_path。", code="missing_config", provider="lighttts")
        if not os.path.exists(prompt_wav_path):
            raise SpeechGatewayError(f"LightTTS prompt_wav 不存在：{prompt_wav_path}", code="missing_prompt_wav", provider="lighttts")
        prompt_text = str(session.get("prompt_text") or "").strip()
        if not prompt_text:
            raise SpeechGatewayError("LightTTS 缺少 prompt_text。", code="missing_config", provider="lighttts")

        # LightTTS 的 /inference_zero_shot 流式模式只接受 speed==1.0；speed!=1.0 + stream 会返回
        # HTTP 400（"speed change only support non-stream inference mode"），这是 LightTTS 接入
        # 失败的首要根因。比赛优先实时性：speed==1.0 时用流式；只有显式需要变速时才退回非流式
        # （非流式服务端才接受 speed），从而保证任何配置下都不会 400。volume / pitch_rate
        # LightTTS 不支持，直接丢弃避免歧义。
        try:
            speed_value = float(session.get("speed") or 1.0)
        except (TypeError, ValueError):
            speed_value = 1.0
        needs_speed = abs(speed_value - 1.0) >= 1e-3
        use_stream = bool(session.get("stream", True)) and not needs_speed
        data = {
            "tts_text": content,
            "prompt_text": prompt_text,
            "stream": "true" if use_stream else "false",
            "tts_model_name": str(session.get("tts_model_name") or "default"),
        }
        if needs_speed:
        ...
        chunk_count = 0
        try:
            async with httpx.AsyncClient(timeout=httpx.Timeout(float(session.get("timeout_s") or 90), connect=8.0)) as client:
                with open(prompt_wav_path, "rb") as prompt_wav:
                    files = {"prompt_wav": ("prompt.wav", prompt_wav, "audio/wav")}
                    async with client.stream("POST", endpoint, data=data, files=files) as response:
                        if response.status_code >= 400:
                            detail = await response.aread()
                            raise SpeechGatewayError(
                                f"LightTTS 返回错误：{response.status_code} {detail[:200].decode('utf-8', 'ignore')}",
                                code=response.status_code,
                                provider="lighttts",
                            )
                        async for chunk in response.aiter_bytes():
            ...
            raise SpeechGatewayError(f"LightTTS 调用失败：{exc}", provider="lighttts") from exc
        # 200 但零音频（极少见：服务端把文本规范化成空、或解码异常）必须当作失败上抛，
        # 让上层走重试 / 跳句降级，绝不归档一段静音去顶替本句发言。
        if chunk_count == 0:
            raise SpeechGatewayError("LightTTS 返回了空音频。", code="empty_audio", provider="lighttts")
        yield {
            "type": "done",
            "mime_type": self.stream_mime_type(**options),
            "latency_ms": int((time.perf_counter() - started) * 1000),
    ...
    def _session_options(self, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        session: Dict[str, Any] = {
            "sample_rate": int(options.get("sample_rate") or preset.get("sample_rate") or settings.get("sample_rate") or 24000),
            "tts_model_name": options.get("tts_model_name") or preset.get("tts_model_name") or settings.get("tts_model_name") or "default",
            "prompt_text": options.get("prompt_text") or preset.get("prompt_text") or settings.get("prompt_text") or "",
            "prompt_wav_path": options.get("prompt_wav_path") or preset.get("prompt_wav_path") or settings.get("prompt_wav_path") or "",

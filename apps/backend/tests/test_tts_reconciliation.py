...
def test_agent_output_budget_uses_lighttts_voice_timing(monkeypatch) -> None:
    from app.services.integration_config import integration_config

    integration_config.config["tts"]["provider"] = "lighttts"
    integration_config.config["tts"]["enabled"] = True
    integration_config._normalize()

    fast_speaker = {
        "id": "spk_fast",
        "speaker_type": "agent",
        "tts_voice_preset_id": "voice_lighttts_debate_1",
    }
    slow_speaker = {
        "id": "spk_slow",
        "speaker_type": "agent",
        "tts_voice_preset_id": "voice_lighttts_debate_3",
    }

    fast = store._agent_output_budget(90, 1.0, speaker=fast_speaker)
    slow = store._agent_output_budget(90, 1.0, speaker=slow_speaker)

    assert fast["raw_chars_per_second"] == 5.8
    assert slow["raw_chars_per_second"] == 3.9
    assert fast["target_chars"] == 522
    assert slow["target_chars"] == 351
    assert fast["min_chars"] == 470
    assert slow["min_chars"] == 316
    assert fast["max_token"] > slow["max_token"]
...
def test_lighttts_pcm_archive_trims_repeated_sentence_silence(monkeypatch) -> None:
    monkeypatch.setenv("PHDEBATE_TTS_TRIM_PCM_SILENCE", "1")
    sample_rate = 24000
    lead = b"\x00\x00" * int(sample_rate * 0.5)
    tail = b"\x00\x00" * int(sample_rate * 0.2)
    tone = b"".join(
        (4000 if idx % 20 < 10 else -4000).to_bytes(2, "little", signed=True)
        for idx in range(int(sample_rate * 0.3))
    )

    wav_bytes = store._pcm16le_to_wav_bytes(lead + tone + tail, sample_rate=sample_rate)

    with wave.open(io.BytesIO(wav_bytes), "rb") as wav:
        duration_s = wav.getnframes() / wav.getframerate()

    assert 0.35 <= duration_s <= 0.5
...
def _stream_selection(provider: str = "alicloud", mime_type: str = "audio/mpeg"):
    class StreamGateway:
        def stream_mime_type(self, **_opts) -> str:
            return mime_type

        async def synthesize_stream(self, text: str, **_opts):  # pragma: no cover - 仅需存在
            yield {"type": "done", "mime_type": mime_type, "latency_ms": 1, "chunk_count": 1}

    return SimpleNamespace(provider=provider, gateway=StreamGateway(), options={})
...
def test_live_stream_default_off(monkeypatch) -> None:
    ...
    assert store._tts_live_mime_type(_stream_selection("lighttts", "audio/L16;rate=24000")) == ""

...
def test_normalize_tts_text_does_not_add_comma_after_sentence_newline() -> None:
    result = normalize_tts_text("大家下午好。\n\n开宗明义，AI推荐算法的核心机制是协同过滤。")

    assert "。，" not in result
    assert result == "大家下午好。开宗明义，AI推荐算法的核心机制是协同过滤。"
...
def test_lighttts_stream_sends_prompt_and_returns_pcm(monkeypatch, tmp_path) -> None:

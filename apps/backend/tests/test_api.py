...
def test_ruleset_legacy_free_debate_duration_maps_to_three_minutes_per_side() -> None:
    phases = store._phases_from_ruleset(
        {
            "flow": [
                {
    ...
    assert phases[0]["duration_seconds"] == 360
    assert phases[0]["side_total_seconds"] == 180
    assert phases[0]["turn_seconds"] == 30
...
def test_free_debate_single_turn_defaults_to_30_seconds(monkeypatch) -> None:
    ...
    free_phase = next(phase for phase in snapshot["phases"] if phase["id"] == "phase_free_debate")
    assert free_phase["duration_seconds"] == 360
    assert free_phase["side_total_seconds"] == 180
    assert free_phase["turn_seconds"] == 30
    turn_clock = next(clock for clock in snapshot["clocks"] if clock["name"] == "turn")
    assert turn_clock["total_seconds"] == 30


def test_legacy_free_debate_default_migrates_to_three_minutes_per_side() -> None:
    free_phase = next(phase for phase in store.snapshot["phases"] if phase["id"] == "phase_free_debate")
    free_phase["duration_seconds"] = 480
    free_phase["side_total_seconds"] = 240
    free_phase["turn_seconds"] = 15
...
def test_clock_pause_resume_and_adjust_flow() -> None:
    ...
    assert 0 <= turn["remaining_ms"] <= 30000
...
def test_agent_text_is_clamped_to_output_budget() -> None:
    ...
    hard_text = "这是一段没有任何标点但明显超过限制的长文本而且还会继续拖得更长"
...
def test_agent_text_clamp_allows_sentence_to_finish_just_past_budget() -> None:
    text = "一二三四五六七八九十继续读完。后面不要读。"

    clipped, clamped = store._clamp_agent_text_to_budget(text, {"target_chars": 10})

    assert clamped is True
    assert clipped == "一二三四五六七八九十继续读完。"
...
def test_agent_payload_carries_max_token_and_message_history() -> None:
    ...
    assert payload["min_len"] == payload["target_min_chars"]
    assert payload["min_length"] == payload["target_min_chars"]
    assert payload["other_info"]["min_chars"] == payload["target_min_chars"]
    assert str(payload["target_chars"]) in payload["length_hint"]

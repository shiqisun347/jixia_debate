...
本章定义 React 前端入口、状态渲染和交互约束。

## 1. 前端入口

| 路径 | 说明 | 实现入口 |
| --- | --- | --- |
| `/screen?match_id=match_001` | 大屏展示页 | `src/pages/ScreenPage.tsx` |
| `/console/:speakerId?match_id=match_001` | 人类辩手控制台 | `src/pages/ConsolePage.tsx` |
| `/admin?match_id=match_001` | 管理端 | `src/pages/AdminPage.tsx` |
| `/vote/:matchId` | 学生投票页 | `src/pages/VotePage.tsx` |
...
实现入口：`apps/frontend/src/pages/ScreenPage.tsx`。
...
- 背景使用 `apps/frontend/public/assets/stage-bg.png` 或同等主题背景，文字区域必须有压暗层。
- 页面只读，无流程控制按钮。
- 所有时间数字使用等宽数字或 `font-variant-numeric: tabular-nums`。

## 3. 辩手控制台

实现入口：`apps/frontend/src/pages/ConsolePage.tsx`。
...
- `pause-speaking` / `resume-speaking`：仅本人 active speech 可点；结束后不能继续。
- `skip-turn`：自由辩论中可点；同方人类都跳过或等待 5 秒无人开始时由 AI 接管。
...
实现入口：`apps/frontend/src/pages/AdminPage.tsx`。
...
- 控制按钮：开始比赛、暂停/继续、回退、跳过、下一环节、重置当前发言、重置比赛、校准时钟、强制结束。
...
- 测试调试区：ASR/TTS 自检、归档识别、Agent 健康检查、接口配置结构化表单、导出包和事件日志集中展示。
- 事件日志：按 `seq` 展示。

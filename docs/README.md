...
本目录是面向实现的**开发规格（development spec）**，把上层需求落地为可直接编码和部署的技术契约。
...
| [20-server-recovery.md](20-server-recovery.md) | 运行态备份与恢复边界 |
| [21-migration-180.md](21-migration-180.md) | 目标服务器迁移和 12234 端口部署流程 |
...
| §6.1 大屏展示页 | 08（scene/mode）、`ScreenPage.tsx` |
| §6.2 辩手控制台 | 08（state）、`ConsolePage.tsx` |
| §6.3 学生投票页 | 09、08 |
| §6.4 管理页面 | 08（三 tab）、`AdminPage.tsx` |
| §7.1 状态机 | 03 |
